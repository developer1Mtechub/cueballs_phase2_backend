...
exports.getAllNotifications = async (req, res, next) => {
  ...
  try {
    ...
    const userCreatedAt = userQuery.rows[0].created_at;
    ...
    let query = `
      ...
      WHERE (
        CASE 
          WHEN user_id IS NULL THEN created_at >= $1
          ELSE user_id = $2
        END
      )
    `;
    let values = [userCreatedAt, user_id];
    ...
    `;
    let countValues = [userCreatedAt, user_id];
