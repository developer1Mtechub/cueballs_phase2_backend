  ...
  try {
    ...
    if (signin_type === "google" || signin_type === "apple") {
      if (signup_type === signin_type) {
        ...
        const result = await pool.query(
          `UPDATE users 
           SET access_token = COALESCE($1, access_token),
               ...
               web_token = COALESCE($3, web_token)
           WHERE user_id = $4 RETURNING *`,
          [access_token, device_token, web_token, user_id]
        );
        ...
        // Update signup method if first login was different
        const result = await pool.query(
          `UPDATE users 
           SET signup_type = $1,
               access_token = COALESCE($2, access_token),
               device_token = COALESCE($3, device_token),
               web_token = COALESCE($4, web_token)
           WHERE user_id = $5 RETURNING *`,
          [signin_type, access_token, device_token, web_token, user_id]
        );
